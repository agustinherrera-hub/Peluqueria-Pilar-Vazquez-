...
const TIEMPOS_TINTE = {
    ...
    'MECHAS BITONO +MAT':           { aplicacion: 90, espera: 15, aclarado: 15, espera2: 15 },
    'MECHAS BALAYGE':               { aplicacion: 90, espera: 30, aclarado: 15, espera2: 30 },
    'MECHS COMP+MAT':               { aplicacion: 90, espera: 15, aclarado: 15, espera2: 15 },
    'MATIZADOR':                    { aplicacion: 30, espera: 30, aclarado: 15, espera2: 0  },
};
...
async function cargarDatosDesdeSheet(silencioso = false) {
    try {
        ...
        // Reinyectar pendientes locales que aún no hayan llegado a Sheets
...
async function eliminarCita(idCita) {
    ...
    // Quitar de memoria local inmediatamente para que desaparezca de la agenda
    // El borrado real en Sheets se hace abajo — cuando los demás dispositivos
    // recarguen datos, la cita ya no estará en Sheets y tampoco aparecerá.
    TODAS_LAS_CITAS = TODAS_LAS_CITAS.filter(c => c.ID_Cita !== idCita);
    // Si era una cita pendiente local, quitarla también del localStorage
    try {
...
function abrirMoverCita(idCita, cliente, horaActual, fechaActual, trabajadoraActual) {
    ...
    async function confirmarMover() {
        ...
        // Cambiar fecha visible si es distinta a la actual (ANTES de repintar)
        ...
        // Repintar agenda con la fecha ya actualizada
        pintarCitasEnAgenda(TODAS_LAS_CITAS);
