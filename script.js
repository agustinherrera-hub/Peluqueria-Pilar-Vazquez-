...
const _citasPendientesSesion = [];       // Citas guardadas aún no confirmadas por Sheets
...
const TIEMPOS_TINTE = {
	...
	'TINTE Y PEINADO':        { aplicacion: 15, espera: 30, aclarado: 30, espera2: 0  },
	...
	'MATIZADOR':  { aplicacion: 15, espera: 30, aclarado: 15, espera2: 0  },
	'MATIZADOR Y PEINADO ':        { aplicacion: 15, espera: 30, aclarado: 30, espera2: 0  },
...
// ─────────────────────────────────────────────
// AUTOCOMPLETE PERSONALIZADO DE CLIENTAS
// position:fixed para escapar del overflow del
// modal. Detecta si hay más espacio arriba o
// abajo del input y abre en la dirección correcta,
// así nunca queda tapado por el teclado virtual.
// ─────────────────────────────────────────────
...
function posicionarDropdown() {
    ...
    if (espacioAbajo >= altoMax || espacioAbajo >= espacioArriba) {
        // Abrir hacia abajo
        dropdown.style.top    = (rect.bottom + 4) + 'px';
        dropdown.style.bottom = 'auto';
        dropdown.style.maxHeight = Math.min(espacioAbajo - 8, altoMax) + 'px';
    } else {
        // Abrir hacia arriba
        dropdown.style.bottom = (vh - rect.top + 4) + 'px';
        dropdown.style.top    = 'auto';
        dropdown.style.maxHeight = Math.min(espacioArriba - 8, altoMax) + 'px';
    }
}

function filtrarAutocompletado(valor) {
    const dropdown = document.getElementById('listaClientesDropdown');
    if (!dropdown) return;

    const texto = (valor || '').trim().toLowerCase();

    if (!texto) { cerrarAutocompletado(); return; }

    const resultados = CLIENTES_MASTER
        .map(c => (`${(c.Nombre||'').trim()} ${(c.Apellidos||'').trim()}`).trim())
        .filter(n => n && n.toLowerCase().includes(texto))
        .slice(0, 30);

    if (resultados.length === 0) { cerrarAutocompletado(); return; }

    dropdown.innerHTML = resultados.map((nombre, i) => {
        const idx      = nombre.toLowerCase().indexOf(texto);
        const antes    = nombre.slice(0, idx);
        const coincide = nombre.slice(idx, idx + texto.length);
        const despues  = nombre.slice(idx + texto.length);
        return `<div class="autocomplete-item" data-nombre="${nombre}" data-idx="${i}">${antes}<mark>${coincide}</mark>${despues}</div>`;
    }).join('');
    ...
    requestAnimationFrame(posicionarDropdown);

    // Reposicionar si el viewport cambia (teclado aparece/desaparece)
    if (window.visualViewport) {
        window.visualViewport.addEventListener('resize', posicionarDropdown, { once: true });
    }

    dropdown.querySelectorAll('.autocomplete-item').forEach(item => {
        // Ratón: preventDefault evita que el input pierda el foco antes de leer el valor
        item.addEventListener('mousedown', (e) => {
            e.preventDefault();
            seleccionarAutocompletado(item.getAttribute('data-nombre'));
        });

        // Táctil: distinguir tap de scroll para no bloquear el deslizamiento
        let _touchStartY = 0;
        item.addEventListener('touchstart', (e) => {
            _touchStartY = e.touches[0].clientY;
        }, { passive: true }); // passive:true → NO bloquea el scroll

        item.addEventListener('touchend', (e) => {
            const dy = Math.abs(e.changedTouches[0].clientY - _touchStartY);
            if (dy < 8) { // menos de 8px → fue un tap, no un scroll
                e.preventDefault();
                seleccionarAutocompletado(item.getAttribute('data-nombre'));
            }
        }, { passive: false });
...
function seleccionarAutocompletado(nombre) {
    const input = document.getElementById('cliente');
    if (input) {
        input.value = nombre;
        verificarNotasCliente(nombre);
        mostrarHistorialClientaEnForm(nombre);
        mostrarProximasCitasEnForm(nombre);
    }
...
function actualizarActivoDropdown(items) {
    items.forEach((item, i) => {
        item.classList.toggle('activo', i === _autocompleteIndiceActivo);
        if (i === _autocompleteIndiceActivo) item.scrollIntoView({ block: 'nearest' });
    });
}
...
window.onload = () => {
    ...
    // Reposicionar dropdown si el teclado virtual cambia el viewport
    if (window.visualViewport) {
    ...
    if (inputCliente) {
        inputCliente.addEventListener('keydown', (e) => {
            ...
            if (e.key === 'ArrowDown') {
                e.preventDefault();
                _autocompleteIndiceActivo = Math.min(_autocompleteIndiceActivo + 1, items.length - 1);
                actualizarActivoDropdown(items);
            } else if (e.key === 'ArrowUp') {
                e.preventDefault();
                _autocompleteIndiceActivo = Math.max(_autocompleteIndiceActivo - 1, 0);
                actualizarActivoDropdown(items);
            } else if (e.key === 'Enter' && _autocompleteIndiceActivo >= 0) {
                e.preventDefault();
                seleccionarAutocompletado(items[_autocompleteIndiceActivo].getAttribute('data-nombre'));
...
async function cargarDatosDesdeSheet(silencioso = false) {
    try {
        ...
        // Rescatar citas pendientes guardadas en memoria ANTES de machacar TODAS_LAS_CITAS
        const pendientesAntes = [..._citasPendientesSesion];
        ...
        // Actualizar el array en memoria (sin localStorage)
        _citasPendientesSesion.length = 0;
        pendientesSiguientes.forEach(p => _citasPendientesSesion.push(p));
        ...
        // ── Actualizar lista de clientes (autocomplete personalizado) ──
        // Los datos ya están en CLIENTES_MASTER; el dropdown se genera dinámicamente en filtrarAutocompletado()
...
async function eliminarCita(idCita) {
    ...
    // Quitar tambien de las pendientes en memoria
    const idx = _citasPendientesSesion.findIndex(p => (p.ID_Cita || '').trim() === idCita.trim());
    if (idx !== -1) _citasPendientesSesion.splice(idx, 1);
...
function calcularDuracion() {
    ...
    if (trabajadora) {
        const horaPrevia = document.getElementById('horaSeleccionada')?.value;
        generarBotonesHora();
        // Restaurar la hora seleccionada si sigue disponible
        if (horaPrevia) {
            const gridHoras = document.getElementById('gridHorasSeleccion');
            const btnHora = gridHoras?.querySelector(`.btn-hora[data-hora="${horaPrevia}"]`);
            if (btnHora) {
                btnHora.click();
            } else {
                document.getElementById('horaSeleccionada').value = horaPrevia;
            }
        }
    }
...
function confirmarServicios() {
    ...
    // Mantener la trabajadora seleccionada para poder añadir más servicios sin tener que volver a elegirla
    ...
    // Regenerar botones de hora excluyendo los tramos del bloque recién confirmado
    generarBotonesHora();
...
if (formulario) {
    formulario.onsubmit = async (e) => {
        ...
        // Para tintes: usa getRangoVisual → los slots de espera quedan LIBRES
        // para que se puedan meter citas durante el tiempo de posado.
        function citasSeSolapan(nuevaInicio, nuevaFin, citaExistente) {
            if (contieneTinte(citaExistente.Servicios)) {
                // Compara tramo a tramo usando el rango visual (sin espera)
                const tramosExistente = getRangoVisual(citaExistente);
                for (let m = nuevaInicio; m < nuevaFin; m += 15) {
                    if (tramosExistente.has(m)) return true;
                }
                return false;
            }
            // Cita normal: rango continuo
            const inicioExistente = horaAMinutos(citaExistente.Hora_Inicio || '00:00');
            const dur = calcularDuracionNormalizada(citaExistente.Duracion_Total);
            const finExistente = inicioExistente + Math.max(Math.ceil(dur/15), 1) * 15;
            return nuevaInicio < finExistente && nuevaFin > inicioExistente;
        }

        function calcularFinCitaParaMsg(cita) {
        ...
        for (const bloque of bloquesConfirmados) {
            ...
            for (const cita of citasDelDia) {
                if (citasSeSolapan(inicioNueva, finNueva, cita)) {
                    const finExistente = calcularFinCitaParaMsg(cita);
                    conflictos.push({
        ...
        try {
            ...
            // Pintar en pantalla inmediatamente y guardar en memoria de sesión
            ...
            for (const bloque of bloquesConfirmados) {
                ...
                _citasPendientesSesion.push(cLocal);
            }
            pintarCitasEnAgenda(TODAS_LAS_CITAS);
            ...
            document.querySelectorAll('.bloque-provisional').forEach(el => el.remove());
            cerrarFormulario();
            // Recargar desde Sheets para sincronizar todos los dispositivos
            cargarDatosDesdeSheet(true);
        } catch (err) {
...
function crearOverlay(cita, tramoHora, tipo, esPrimero, anchoPct, offsetPct, horaFin, duracion) {
    ...
    if (tipo === 'espera' || tipo === 'espera2') {
        ...
        const botonLupa = `<button onclick="event.stopPropagation();verDetalleCita('${cita.ID_Cita}')" style="margin-left:2px;background:rgba(255,255,255,0.25);border:none;border-radius:3px;color:white;font-size:9px;padding:0 4px;cursor:pointer;flex-shrink:0;">🔍</button>`;
        const tieneNota = !!(localStorage.getItem('nota_cita_' + cita.ID_Cita) || '').trim();
        const botonLapiz = `<button onclick="event.stopPropagation();abrirNotaCita('${cita.ID_Cita}')" title="Añadir nota" style="margin-left:2px;background:${tieneNota ? 'rgba(255,255,150,0.55)' : 'rgba(255,255,255,0.25)'};border:none;border-radius:3px;color:white;font-size:9px;padding:0 4px;cursor:pointer;flex-shrink:0;">✏️</button>`;
        overlay.innerHTML = `
            ...
            </span>${botonLapiz}${botonLupa}${botonMover}${boton}`;
    } else {
                ...
                for (let i = 0; i < tramosAPintar; i++) {
                    ...
                    if (usarOverlay) {
                        ...
                        if (tipo === 'espera') {
                            ...
                            // Slot de espera: clicable para meter nueva cita
                            if (!slot._esperaClickActivo) {
                                slot._esperaClickActivo = true;
                                slot.addEventListener('click', (ev) => {
                                    ev.stopPropagation();
                                    const tObj = TRABAJADORAS.find(t => colIdParaTrabajadora(t.Nombre) === colId);
                                    const nombreTrab = tObj ? tObj.Nombre : (cita.Trabajadora || '');
                                    mostrarMenuSlot(colId, tramoHora, nombreTrab, slot);
                                });
                            }
                        } else {
                            const colorBase = colorParaClientа(cita.ID_Cliente);
...
// Cargar bloqueos solo desde Sheets (sin localStorage — multi-dispositivo)
function cargarBloqueos() {
    SLOTS_BLOQUEADOS.clear();
    // Fuente de verdad: solo RESERVAS_SHEETS
...
function guardarBloqueos() {
    // Sin localStorage — los bloqueos se sincronizan via Google Sheets
}
...
// Abre el formulario con trabajadora Y hora preseleccionadas desde el slot.
// El foco va al nombre de la clienta (sin scroll al grid de horas).
function abrirFormularioDesdeSlot(horaStr, nombreTrabajadora) {
    ...
    setTimeout(() => {
        ...
        if (tObj) seleccionarTrabajadora(tObj.Area || tObj.Nombre, tObj.Nombre);

        setTimeout(() => {
            // Marcar hora silenciosamente (sin scroll)
            const gridHoras = document.getElementById('gridHorasSeleccion');
            if (gridHoras) {
                gridHoras.querySelectorAll('.btn-hora.active').forEach(b => b.classList.remove('active'));
                const btnHora = gridHoras.querySelector(`.btn-hora[data-hora="${horaStr}"]`);
                if (btnHora && !btnHora.classList.contains('btn-hora-ocupada')) {
                    btnHora.classList.add('active');
                }
            }
            document.getElementById('horaSeleccionada').value = horaStr;
            // Foco al nombre de la clienta
            const inputCliente = document.getElementById('cliente');
...
function pintarSlotBloqueado(slotEl, nota, key) {
    ...
    over.style.cssText = `
        ...
        z-index:11;cursor:default;box-sizing:border-box;
...
// ─────────────────────────────────────────────
// VER DETALLE DE CITA — POPUP INFORMATIVO
// ─────────────────────────────────────────────
function verDetalleCita(idCita) {
    const cita = TODAS_LAS_CITAS.find(c => (c.ID_Cita || '').trim() === idCita.trim());
    if (!cita) return;

    // Buscar teléfono de la clienta
    const nombreLower = (cita.ID_Cliente || '').trim().toLowerCase();
    const clienteObj = CLIENTES_MASTER.find(c =>
        `${(c.Nombre||'').trim()} ${(c.Apellidos||'').trim()}`.trim().toLowerCase() === nombreLower
    );
    const tel = clienteObj
        ? (clienteObj.Telefono || clienteObj.Tel || clienteObj.Teléfono ||
           clienteObj.telefono || clienteObj.TELEFONO || clienteObj.Movil ||
           clienteObj.Móvil || clienteObj.movil || '').toString().trim()
        : '';

    // Calcular hora fin
    const inicioMin = horaAMinutos(cita.Hora_Inicio || '00:00');
    const duracion = calcularDuracionNormalizada(cita.Duracion_Total);
    const horaFin = minutosAHora(inicioMin + Math.max(Math.ceil(duracion / 15), 1) * 15);
    ...
    const serviciosList = (cita.Servicios || '')
        ...
        .map(s => s.trim())
        .filter(Boolean)
        .map(s => `<div style="display:flex;align-items:center;gap:6px;padding:4px 0;border-bottom:1px solid #fce7f3;">
            <span style="width:6px;height:6px;border-radius:50%;background:#ec4899;flex-shrink:0;display:inline-block;"></span>
            <span style="font-size:0.82rem;font-weight:600;color:#1e293b;">${s}</span>
        </div>`)
        .join('');

    // Eliminar popup anterior
    const existente = document.getElementById('popupDetalleCita');
    if (existente) existente.remove();
    const existenteOv = document.getElementById('overlayDetalleCita');
    if (existenteOv) existenteOv.remove();

    const overlay = document.createElement('div');
    overlay.id = 'overlayDetalleCita';
    overlay.style.cssText = 'position:fixed;inset:0;background:rgba(15,23,42,0.45);z-index:9998;backdrop-filter:blur(2px);';

    const popup = document.createElement('div');
    popup.id = 'popupDetalleCita';
    popup.style.cssText = `
        position: fixed;
        z-index: 9999;
        background: white;
        border-radius: 1.4rem;
        box-shadow: 0 20px 60px rgba(236,72,153,0.22), 0 4px 16px rgba(0,0,0,0.12);
        padding: 0;
        width: 300px;
    ...
    const idCitaEscape = (cita.ID_Cita || '').replace(/'/g, "\'");

    popup.innerHTML = `
        <!-- Cabecera rosa -->
        <div style="background:linear-gradient(135deg,#ec4899,#f472b6);padding:1rem 1.2rem 0.8rem;color:white;">
            <div style="display:flex;justify-content:space-between;align-items:flex-start;">
                <div>
                    <div style="font-size:0.65rem;font-weight:800;text-transform:uppercase;opacity:0.85;letter-spacing:0.05em;margin-bottom:2px;">Detalle de cita</div>
                    <div style="font-size:1.1rem;font-weight:900;line-height:1.2;">${cita.ID_Cliente || ''}</div>
                </div>
                <button id="btnCerrarDetalle" style="background:rgba(255,255,255,0.25);border:none;border-radius:50%;width:28px;height:28px;color:white;font-size:1rem;cursor:pointer;display:flex;align-items:center;justify-content:center;flex-shrink:0;">✕</button>
            </div>
            ${tel ? `
            <div style="margin-top:0.6rem;display:flex;align-items:center;gap:6px;">
                <span style="font-size:0.75rem;opacity:0.85;">📱</span>
                <a href="tel:${tel.replace(/\s/g,'')}" style="font-size:0.85rem;font-weight:700;color:white;text-decoration:none;opacity:0.95;">${tel}</a>
            </div>` : ''}
        </div>
        ...
        <div style="padding:1rem 1.2rem;">
            ...
            <div style="display:flex;gap:8px;margin-bottom:0.9rem;">
                <div style="flex:1;background:#fdf2f8;border-radius:0.75rem;padding:0.5rem 0.7rem;">
                    <div style="font-size:0.6rem;font-weight:800;color:#ec4899;text-transform:uppercase;margin-bottom:2px;">Fecha</div>
                    <div style="font-size:0.78rem;font-weight:700;color:#1e293b;line-height:1.3;">${fechaFormateada}</div>
                </div>
                <div style="background:#fdf2f8;border-radius:0.75rem;padding:0.5rem 0.7rem;min-width:80px;text-align:center;">
                    <div style="font-size:0.6rem;font-weight:800;color:#ec4899;text-transform:uppercase;margin-bottom:2px;">Hora</div>
                    <div style="font-size:0.85rem;font-weight:900;color:#1e293b;">${cita.Hora_Inicio} – ${horaFin}</div>
                    ${duracion ? `<div style="font-size:0.65rem;color:#94a3b8;font-weight:600;">${duracion} min</div>` : ''}
                </div>
            </div>

            <!-- Servicios -->
            <div style="margin-bottom:0.9rem;">
                <div style="font-size:0.6rem;font-weight:800;color:#ec4899;text-transform:uppercase;margin-bottom:0.4rem;">Servicios</div>
                <div style="background:#fdf2f8;border-radius:0.75rem;padding:0.4rem 0.7rem;">
                    ${serviciosList || '<span style="font-size:0.8rem;color:#94a3b8;">Sin servicios registrados</span>'}
                </div>
            </div>

            <!-- Trabajadora -->
            <div style="background:#fdf2f8;border-radius:0.75rem;padding:0.5rem 0.7rem;display:flex;align-items:center;gap:8px;margin-bottom:0.9rem;">
                <div>
                    <div style="font-size:0.6rem;font-weight:800;color:#ec4899;text-transform:uppercase;">Trabajadora</div>
                    <div style="font-size:0.85rem;font-weight:700;color:#1e293b;">${cita.Trabajadora || '—'}</div>
                </div>
            </div>

            <!-- Notas de la cita -->
            <div>
                <div style="font-size:0.6rem;font-weight:800;color:#ec4899;text-transform:uppercase;margin-bottom:0.4rem;">✏️ Notas de la cita</div>
                <textarea id="textareaNotaDetalle"
                    placeholder="Escribe aquí apuntes, observaciones o recordatorios de esta cita..."
                    style="width:100%;min-height:80px;resize:vertical;background:#fffbf0;border:1.5px solid #fde68a;border-radius:0.75rem;padding:0.55rem 0.7rem;font-size:0.82rem;font-weight:500;color:#1e293b;font-family:inherit;outline:none;box-sizing:border-box;"
                    onfocus="this.style.borderColor='#f59e0b'"
                    onblur="this.style.borderColor='#fde68a'"
                >${(localStorage.getItem('nota_cita_' + idCitaEscape) || '')}</textarea>
                <button id="btnGuardarNota"
                    style="margin-top:0.5rem;width:100%;background:linear-gradient(135deg,#f59e0b,#fbbf24);color:white;border:none;padding:0.55rem;border-radius:0.75rem;font-weight:900;font-size:0.8rem;cursor:pointer;">
                    💾 GUARDAR NOTA
                </button>
            </div>
        </div>
    ...
    if (btnGuardar) {
        btnGuardar.onclick = () => {
            const texto = (document.getElementById('textareaNotaDetalle').value || '').trim();
            if (texto) {
                localStorage.setItem('nota_cita_' + cita.ID_Cita, texto);
            } else {
                localStorage.removeItem('nota_cita_' + cita.ID_Cita);
            }
            // Feedback visual
            btnGuardar.textContent = '✅ GUARDADO';
            btnGuardar.style.background = 'linear-gradient(135deg,#10b981,#34d399)';
            setTimeout(() => {
                cerrarDetalle();
                pintarCitasEnAgenda(TODAS_LAS_CITAS); // refrescar para actualizar color del lápiz
            }, 700);
        };
    }
}
...
// ─────────────────────────────────────────────
// ABRIR NOTA RÁPIDA DE CITA (desde botón lápiz)
// ─────────────────────────────────────────────
function abrirNotaCita(idCita) {
    const cita = TODAS_LAS_CITAS.find(c => (c.ID_Cita || '').trim() === idCita.trim());
    if (!cita) return;

    // Reutilizar verDetalleCita que ya tiene el textarea de notas
    verDetalleCita(idCita);

    // Hacer scroll al textarea y darle foco tras un tick
    setTimeout(() => {
        const ta = document.getElementById('textareaNotaDetalle');
        if (ta) {
