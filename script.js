...
function mostrarPopupBloqueo(colId, horaStr, trabajadora, slotEl) {
    ...
    popup.querySelector('#btnNuevaCitaDesdeSlot').onclick = () => {
        cerrarPopup();
        // Abrir formulario con la hora y trabajadora preseleccionadas
        abrirFormulario();
        // Esperar a que el modal esté visible y luego preseleccionar
        setTimeout(() => {
            // Preseleccionar trabajadora
            if (trabajadora) {
                const tObj = TRABAJADORAS.find(t => t.Nombre === trabajadora);
                if (tObj) seleccionarTrabajadora(tObj.Area || tObj.Nombre, tObj.Nombre);
            }
            // Regenerar botones de hora y seleccionar la hora del slot
            generarBotonesHora();
            setTimeout(() => {
                const btnHora = document.querySelector(`[data-hora="${horaStr}"]`);
                if (btnHora) {
                    ...
                    document.querySelectorAll('.btn-hora').forEach(b => b.style.cssText = estiloInactivo);
