...
function generarBotonesHora() {
    ...
    // Para cada slot: mostrar siempre, pero marcar los ocupados/sin hueco como no seleccionables
    let htmlBotones = '';
    todosLosSlots.forEach(minInicio => {
        ...
        if (ocupado) {
            // Slot ocupado: visible pero bloqueado
            htmlBotones += `<div class="btn-hora btn-hora-ocupada" data-hora="${horaStr}" data-ocupada="1" aria-disabled="true" title="Ocupado">${horaStr}</div>`;
            return;
        }
        ...
        if (caben) {
            ...
            // Libre pero sin hueco suficiente: visible pero bloqueado
            htmlBotones += `<div class="btn-hora btn-hora-ocupada" data-hora="${horaStr}" data-ocupada="1" aria-disabled="true" title="Sin hueco suficiente">${horaStr}</div>`;
        }
    });
    ...
    contenedor.onclick = (e) => {
        ...
        if (botonPulsado && !botonPulsado.dataset.ocupada) seleccionarBoton(botonPulsado);
    };
...
function confirmarServicios() {
    ...
    // Resetear botones hora y regenerar para reflejar bloques confirmados como ocupados
    generarBotonesHora();
