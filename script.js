    ...
    try {
        ...
        pendientesAntes.forEach(p => {
            const yaEnSheet = TODAS_LAS_CITAS.some(c =>
                c.Fecha === p.Fecha &&
                (c.Hora_Inicio || '').padStart(5, '0') === (p.Hora_Inicio || '').padStart(5, '0') &&
                (c.ID_Cliente || '').trim().toLowerCase() === (p.ID_Cliente || '').trim().toLowerCase() &&
...
if (formulario) {
    formulario.onsubmit = async (e) => {
        ...
        try {
            ...
            // Esperar 2 segundos para que Sheets procese la escritura
            ...
            for (const bloque of bloquesConfirmados) {
                const cLocal = {
                    ...
                    Fecha:         fecha,
                    Hora_Inicio:   bloque.hora.padStart(5, '0'),
                    ID_Cliente:    cliente,
                    Trabajadora:   bloque.trabajadora,
                    Area:          bloque.area,
                    Servicios:     bloque.servicios.join(', '),
            ...
            bloquesConfirmados = [];
            cerrarFormulario();
            // Forzar refresco inmediato para que todos los dispositivos vean la cita
