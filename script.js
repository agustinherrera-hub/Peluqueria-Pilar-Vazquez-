...
function mostrarPopupBloqueo(colId, horaStr, trabajadora, slotEl) {
    ...
    popup.innerHTML = `
        ...
        <button id="btnNuevaCitaDesdeSlot" style="width:100%;background:linear-gradient(135deg,#ec4899,#f472b6);color:white;border:none;padding:0.5rem 0.6rem;border-radius:0.6rem;font-size:0.85rem;font-weight:900;cursor:pointer;margin-bottom:0.5rem;text-align:left;">
            📅 NUEVA CITA AQUÍ
        </button>
        <div style="border-top:1px solid #f1f5f9;padding-top:0.5rem;">
        <div style="font-size:0.65rem;font-weight:800;color:#94a3b8;text-transform:uppercase;margin-bottom:0.4rem;">🔒 Bloquear slot</div>
        ...
        <div style="display:flex;gap:0.4rem;">
            ...
            <button id="btnConfirmarBloqueo" style="flex:2;background:#64748b;color:white;border:none;padding:0.5rem;border-radius:0.6rem;font-weight:900;font-size:0.8rem;cursor:pointer;">
                🔒 RESERVAR
            </button>
        </div>
        </div>
    ...
    popup.querySelector('#btnNuevaCitaDesdeSlot').onclick = () => {
        ...
        setTimeout(() => {
            // 1. Preseleccionar trabajadora — esto llama generarBotonesHora internamente
            if (trabajadora) {
                const tObj = TRABAJADORAS.find(t => t.Nombre === trabajadora);
                if (tObj) seleccionarTrabajadora(tObj.Area || tObj.Nombre, tObj.Nombre);
            }

            // 2. Ahora que los botones están generados, marcar la hora
            const estiloActivo   = 'padding:0.5rem;text-align:center;font-size:0.75rem;background:#ec4899;color:white;border:2px solid #ec4899;border-radius:0.5rem;cursor:pointer;font-weight:700;';
            const estiloInactivo = 'padding:0.5rem;text-align:center;font-size:0.75rem;background:white;color:#1e293b;border:1px solid #e2e8f0;border-radius:0.5rem;cursor:pointer;font-weight:400;';
            const contenedorHoras = document.getElementById('gridHorasSeleccion');
            contenedorHoras.querySelectorAll('.btn-hora').forEach(b => b.style.cssText = estiloInactivo);
            const btnHora = contenedorHoras.querySelector(`[data-hora="${horaStr}"]`);
            if (btnHora) {
                btnHora.style.cssText = estiloActivo;
...
function abrirMoverCita(idCita, cliente, horaActual, fechaActual, trabajadoraActual) {
    ...
    async function confirmarMover() {
        ...
        const citaLocal = TODAS_LAS_CITAS.find(c => (c.ID_Cita || '').trim() === idCita.trim());
        if (citaLocal) {
            citaLocal.Fecha = nuevaFecha;
            citaLocal.Hora_Inicio = nuevaHora;
            if (nuevaTrabajadora) citaLocal.Trabajadora = nuevaTrabajadora;
        }

        // Repintar agenda
        pintarCitasEnAgenda(TODAS_LAS_CITAS);

        // Cambiar fecha visible si es distinta a la actual
        ...
        fetch(URL_SCRIPT + '?payload=' + _payload)
            .catch(err => console.error('Error moviendo cita en Sheets:', err));
    }
